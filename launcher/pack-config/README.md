...
## Три независимые выкладки

В релизе с тегом `pack` лежат три несвязанные вещи, и каждая
выкладывается своей командой. Пересобирать остальные при этом не надо.

| Что | Команда | Когда |
|-----|---------|-------|
| Пак | `node launcher/tools/publish-pack.js …` | поменялись моды, конфиги, ресурспаки |
| Лаунчер | `npm run dist`, затем `node launcher/tools/publish-launcher.js …` | поменялся сам лаунчер |
| Новости | `npm run news:publish -- --repo … --token …` | есть что сказать игрокам |

`publish-launcher.js` заливает не только `.exe`, но и описание релиза
`launcher-release.json` — версию, размер и хеш он считает сам. По этому
описанию лаунчер игрока и понимает, что вышла новая версия, поэтому
выкладывать `.exe` в обход этой команды нельзя.

Новости живут в `launcher/news/news.json` и правятся руками. Заготовки
по коммитам можно получить командой `npm run news:draft` — они помечены
`"draft": true`, и выкладка отказывается работать, пока пометка
не снята: технический текст из коммита игроку показывать нечего.
Ограничения одной записи — заголовок до 60 знаков, текст до 400,
