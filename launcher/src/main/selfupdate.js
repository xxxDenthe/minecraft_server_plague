...
// cmd дожидается установщика и стартует .exe из той же папки, куда
// он только что поставил новую версию.
//
// `&`, а не `&&`: сорвавшаяся установка не должна оставить игрока
// без лаунчера вообще — поднимется прежняя версия, и это лучше пустого
// рабочего стола.
export function restartCommand(installer, exe = process.execPath) {
  return {
    command: process.env.COMSPEC ?? 'cmd.exe',
    // windowsVerbatimArguments при spawn: строку ниже собирает cmd,
    // и лишнее экранирование от Node ломает кавычки вокруг путей.
    args: ['/d', '/s', '/c', `"${installer}" /S & start "" "${exe}"`],
  };
}
