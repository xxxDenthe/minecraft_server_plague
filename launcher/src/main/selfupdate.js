...
// Тихая установка NSIS новую версию не поднимает: лаунчер выходит,
// ставится обновление, и на экране не остаётся ничего — игрок читает
// это как краш (так и прочитал). Поэтому перезапуск берём на себя.
//
// Всё это делает .cmd-файл рядом с установщиком, а не строка в аргументах
// cmd /c: правила cmd о крайних кавычках съедали кавычки вокруг путей,
// и команда не выполнялась вовсе — 0.2.2 и 0.2.3 так ни у кого и не
// встали. Файл на диске от этих правил свободен, и его видно глазами,
// когда обновление снова пойдёт не так.
//
// Пауза обязательна: установщик NSIS, увидев запущенный лаунчер, при /S
// молча выходит с кодом 0 и не ставит ничего (проверено 2026-09-13 —
// лаунчер качал обновление каждый старт и оставался прежней версии).
// app.quit() к моменту запуска скрипта ещё не закончился.
//
// `&&` между установкой и запуском нет намеренно: сорвавшаяся установка
// не должна оставить игрока без лаунчера вообще — поднимется прежняя
// версия, и это лучше пустого рабочего стола.
const WAIT_SECONDS = 6;

export function restartScript(installer, exe = process.execPath) {
  return [
    '@echo off',
...
export async function writeRestartScript(installer, exe = process.execPath) {
  const file = path.join(installerDir(), 'restart.cmd');
  await fs.writeFile(file, restartScript(installer, exe), 'utf8');
  return file;
}
