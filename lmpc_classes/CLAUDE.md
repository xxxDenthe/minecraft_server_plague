...
- `client/ClassCodexScreen.java` — свой `Screen`, не ванильная книга:
  тёмная пергаментная панель, четыре вкладки-кнопки сверху (по одной
  на класс), открыт по умолчанию — твой текущий. Мастерство (полоса
  0–100 + число) рисуется только на вкладке твоего класса — у чужих
  вкладок только роль и лор, чтобы не путать «характеристику» с
  «энциклопедией».
