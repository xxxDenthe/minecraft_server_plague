...
`clerics_brew`, кроме питья себя.

- `client/BrewTargeting.java` — держишь отвар в руке, смотришь на
  игрока: он обводится сквозь стены (рефлексия на `Entity.
  setSharedFlag(6, …)`, тот же приём, что ночное зрение у
  `lmpc_gmtools`, — `setGlowingTag` рассчитан на серверный синк
  и для чисто клиентского эффекта не годится) и над головой всплывает
  «⚕ ПКМ — напоить» (`RenderNameTagEvent`, `setCanRender(TRUE)`,
  форсит нетэг даже если у игрока он выключен). Держишь ПКМ —
  вместо надписи растёт текстовый прогресс-бар (`█████░░░░░`, 10
  сегментов). Цель сдвинулась больше чем на пол-блока от точки,
  где начал жать, — прогресс сгорает и отсчёт стартует заново.
  Дошёл до конца (`clericFeedChannelTicks`, умолчание 60 тиков) —
  клиент шлёт `ClassNetwork.FeedRequest`, дальше решает сервер.
- `ClassNetwork.java` — сервер не доверяет клиенту: заново проверяет
  класс отправителя, дистанцию (6 блоков), что отвар физически
  в руке, и кулдаун **цели** (`отварГотовТик` — тот же счётчик,
  что у самостоятельного питья, только теперь про того, кого лечат,
