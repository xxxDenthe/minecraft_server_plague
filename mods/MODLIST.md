...
lmpc_classes-0.8.0.jar    четыре класса, требует curios
```
...
`lmpc_classes` 0.8.0 — все четыре класса играбельны: Клирик, Кузнец,
Фермер, Летописец. Жёстко требует `curios`, без него не грузится.
Очиститель работает по площади 3 × 3 чанка и снимает два-три уровня
за ночь — заметка `2026-09-06-ochistitel-oblast-i-skorost.md`.
