...
**2026-09-12:** взяты два мода под таверну — `emotecraft` (эмоции
и жесты по колесу: чокнуться, сплясать, раскланяться) и `etched`
(патефон и свои пластинки вместо ванильных дисков). Решение владельца:
сервер выходил слишком ванильным и строгим для весёлой компании,
таверна делается местом для посиделок без игровых бонусов. Третий
кандидат, `handcrafted` (мебель, кружки, посуда), уже стоял в профиле
...
```
plaguecore-0.6.0.jar      ядро чумы, сервер + клиент
lmpc_gmtools-0.21.0.jar   панель мастера игры
lmpc_shade-0.9.0.jar      цветокор, тьма, туман
lmpc_classes-0.20.0.jar   четыре класса, требует curios И create
```
...
`lmpc_classes` 0.20.0 — все четыре класса играбельны: Клирик, Кузнец,
...
снизу, а не с любой стороны. В 0.20.0 Летописец перестал видеть точную стадию и заражённость
соседей: вместо чисел — направление («хуже», «легче», «без перемен»)
за две минуты, а тир мастерства даёт память об ушедших. Обе его
панели одеты в кожу полевого лазарета, как экран здоровья на `Y`.
Заметки `2026-09-06-ochistitel-oblast-i-skorost.md`
и `2026-09-06-kurilnica-i-forsazh.md`,
`2026-09-06-ochistitel-create-style.md`,
`2026-09-17-letopisec-napravlenie-i-lazaret.md`.

`plaguecore` 0.6.0 и `lmpc_gmtools` 0.21.0 — хоррор: режиссёр
напряжения. Мир пугает сам, по ритму: на каждого игрока копится
напряжение от темноты, одиночества, Гнили, глубины, ночи и своей стадии,
а после каждого события держится долина тишины. События трёх цен —
шорох за спиной (ванильные звуки, слышит один), видение (темнота,
сердцебиение, силуэт на краю зрения) и явление Наблюдателя. Наблюдатель
стоит, пока на него смотрят, оказывается ближе, стоит отвести глаза,
и растворяется вблизи; последнее из трёх явлений за сессию настоящее
и может убить. Всё правится в секции `[dread]` конфига, выключатель
`dread.enabled`. Пульт мастера — ряд кнопок в карточке игрока и папка
«Страх» в разделе «Чума». Спек
`docs/superpowers/specs/2026-09-17-horror-rezhissyor-design.md`.

`plaguecore` 0.5.0 и `lmpc_gmtools` 0.20.0 — пометки Мастера игры
в экране здоровья. ГМ открывает экран любого игрока кнопкой «Состояние
здоровья…» в его карточке и правит то, что игрок там читает: ставит
готовые состояния (перелом, ожог, лихорадка и ещё семь) или свою строку,
заменой автотекста или строкой ниже, снимает по одной и возвращает всё
как было. Пометки видит и сам игрок, и сосед при осмотре, и Клирик —
подробнее. Живут до снятия или до смерти. Ни одной новой игровой
механики: пометка — это только текст. Спек
`docs/superpowers/specs/2026-09-16-pometki-gm-zdorovye-design.md`.

`plaguecore` 0.4.2 — у `/plague center` появился третий аргумент:
сторона квадрата чумы в чанках, до 511. Квадрат раздувается вокруг
указанной точки, старое заражение остаётся внутри, по краю появляется
чистая земля — так эпидемия дотягивается туда, куда раньше не доставала.
Карта в `/plague gui` прореживается, если квадрат не влезает на экран:
клетка показывает наибольший уровень своего квадратика чанков.

`plaguecore` 0.4.1 — `/plague center` переносит квадрат чумы, а не
стирает его: заражение, шрамы и сопротивление переезжают по абсолютным
координатам чанка, за бывшим краем появляется чистая земля, и эпидемии
снова есть куда расти. Границу мира команда больше не трогает (ставится
ванильным `/worldborder`). В шапке `/plague gui` видно, где стоит центр
и какие чанки покрыты, а открытый экран обновляется сразу после
команды. Бюджет ночи масштабируется по площади сетки.

`plaguecore` 0.4.0 — записи архивов: свой предмет `archive_record`
с начинкой подписанной книги, три вида по архивам (бумаги управы,
церковная тетрадь, журнал дренажа), кафедра и резная полка. Экран
чтения переделан в старую бумагу: своя текстура разворота с
разлиновкой, чернильные уголки вместо кнопок, номер листа вместо
«Страница 1 из 3». Тайнопись перешла на собственный алфавит вместо
рун стола зачарований. Двенадцать шифрованных страниц и десять
записок выдаются `/function plaguecore:zapisi/vse`. Спек
`2026-09-12-zapis-predmet-design.md`.

`plaguecore` 0.3.0 — Пограничье: чанки уровня 1..3 между городом
и Гнилью ожили. Приметы (звуки и пепел тем чаще, чем ближе Гниль),
лут по глубине (сундуки богатеют с уровнем чанка) и ночной прилив
(на закате из ближайшего чанка Гнили выходит волна). Проверка
командами `/plague tide` и `/plague omen`. Заметка
`2026-09-10-pogranichye-kod.md`.
