...
**Решение (после трёх неудачных итераций шейдера, 0.7.1–0.7.3):**
попытки закрыть дырку от `SkyType.NONE` в Fancy пост-шейдером воевали
с рендер-модами пака (Veil, Sable, Flywheel) — небо чинилось, но
дальние облака чернели, фейд давал статичную «плёнку». Шейдер откачен
к 0.7.0 (`lmpc_shade` 0.7.4). **Пак идёт на графике Fabulous**: лаунчер
пишет `options.txt` с `graphicsMode:2` при первом запуске
(`launch.js`, только если файла ещё нет — настройки игрока не трогаем).
У кого инстанс уже создан (владелец, друг) — выставить «Ультра» вручную
один раз. Джар 0.7.4 в паке (`packVersion` 6).
