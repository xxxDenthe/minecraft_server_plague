...
public final class PlagueOverlay {
    ...
    public static void нарисовать(RenderGuiEvent.Post событие) {
        ...
        // Темнота видения приходит и здоровому: это обман чувств,
        // а не симптом, и к плотности плёнки она не привязана.
        float темнота = DreadClient.силаТемноты();
        boolean больной = стадия >= 2 && стадия < ПЛОТНОСТЬ.length;
        if (!ведут && !больной && темнота <= 0f) return;
        ...
        float плотность = ведут ? ОДЕРЖИМОСТЬ : (больной ? ПЛОТНОСТЬ[стадия] : 0f);
        float размах = ведут ? РАЗМАХ_ПУЛЬСА : РАЗМАХ;
        float период = ведут ? 6f : 25f;

        float дыхание = Mth.sin(такт / период) * размах;
        // Побеждает то, что темнее: видение не должно быть слабее
        // собственной плёнки больного, иначе его просто не заметят.
        float своё = больной || ведут ? Mth.clamp(плотность + дыхание, 0f, 0.85f) : 0f;
        залить(графика, 0x000000, Math.max(своё, темнота));
    }
