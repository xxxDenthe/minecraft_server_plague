...
public final class Marks {
     ...
     *
     * @param id номер пометки, от которой строка, или −1 у автотекста
     *           и у второй строки Клирика. По нему редактор ГМ знает,
     *           у какой строки рисовать крестик, а у какой — нечего
     *           снимать.
     */
    public record Вывод(int id, String ключ, String текст, boolean тусклый) {}
    ...
    public static List<Вывод> строки(Место место, String автоКлюч, List<Пометка> пометки,
        ...
        if (автоКлюч != null && !заменяет(место, пометки)) {
            вывод.add(new Вывод(-1, автоКлюч, null, false));
        }
        for (Пометка п : пометки) {
            if (п.место() != место) continue;
            if (п.своя()) {
                вывод.add(new Вывод(п.id(), null, чистить(п.текст()), false));
                continue;
            }
            Заготовка з = заготовка(п.заготовка());
            if (з == null) continue;                 // незнакомая молчит, а не показывает id
            вывод.add(new Вывод(п.id(), ключ(з, чужой), null, false));
            // Строка Клирика идёт без номера: снимать её отдельно нечего,
            // она уйдёт вместе со своей пометкой.
            if (клирик) вывод.add(new Вывод(-1, ключКлирика(з), null, true));
        }
